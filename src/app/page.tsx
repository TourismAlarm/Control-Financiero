...
export default function Home() {
  logger.log('🔵 PAGE.TSX - Componente Home renderizado')
  ...
  logger.log('🔵 PAGE.TSX - Status:', status)
  logger.log('🔵 PAGE.TSX - Session:', session)

  useEffect(() => {
    if (status === 'unauthenticated') {
      logger.log('🔴 PAGE.TSX - Usuario no autenticado, redirigiendo a signin')
      router.push('/auth/signin');
    }
  }, [status, router]);

  if (status === 'loading') {
    logger.log('🟡 PAGE.TSX - Mostrando pantalla de carga')
    return (
  ...
  if (!session) {
    logger.log('🔴 PAGE.TSX - No hay sesión, retornando null')
    return null;
  }

  logger.log('✅ PAGE.TSX - Renderizando app modular')
